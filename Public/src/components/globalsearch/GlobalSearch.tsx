...
import { IonGrid, IonRow, IonCol, IonList, IonItem, IonText, IonAvatar, IonImg, IonChip, IonLabel, IonIcon, IonInput } from "@ionic/react";
...
function GlobalSearch({ searchKeyWord }: Props) {
    ...
    return (
        <>
            {globalData &&
                <IonGrid hidden={resultData.length === 0 && globalDataErr === ""} className={`${searchKeyWord.length > 1 ? 'global-search-grid' : ''}`}>
                    <IonRow>
