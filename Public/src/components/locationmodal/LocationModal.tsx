...
function LocationModel({ isValid, locationlist, showModel }: Props) {
    ...
    const selectLocationHandler = (value: any) => {
        ...
        if (value?.Id > 0) {
            ...
            let data = filterLocation.find(x => x?.Name?.toLowerCase() == value?.toLowerCase());
    ...
    const filterOptions = createFilterOptions({
        stringify: (option: any) => getUserLanguage() === "ae_ar" ? option.SecondLanguage : option.Name + option.Code,
    });
            ...
            {isIn() ?
                <IonGrid style={{ width: '100%', height: '100%', backgroundColor: '#FFFFFF' }} className="locaton-modal-grid" dir={Direction()}>
                    <IonRow>
                        ...
                        <IonCol  className="location-col" size="8" sizeXs="12" sizeSm="11" sizeMd="10.5" sizeLg="8">
                            <Swiper  slidesPerView={(isExtraSmall ? 2 : IsMobile ? 3 : IsTablet ? 5 : 5)} rewind={true}
                                ...
                                {state.map((val, i) => (
                                    <SwiperSlide key={i}>
                                        <IonCard className={`location-card cursor-pointer ${val.Identifier === selectedCity && 'selected-dist'}`} onClick={() => selectedCityHandler(val.Identifier)}>
                ...
                <IonGrid style={{ width: '100%', height: '100%', backgroundColor: '#FFFFFF' }} className="locaton-modal-grid" dir={Direction()}>
                    <IonRow>
                        ...
                        <IonCol size="2" sizeXs="0" sizeSm="1" sizeMd="0.75" sizeLg="2"></IonCol>
                        <IonCol className="location-col" size="8" sizeXs="12" sizeSm="11" sizeMd="10.5" sizeLg="8">
                            <Swiper dir={Direction()} slidesPerView={(isExtraSmall ? 2 : IsMobile ? 2.8 : IsTablet ? 5 : 5)} rewind={true}
                                modules={[Navigation, Pagination]} pagination={{ clickable: true }} navigation={{
                                    prevEl: navigationPrevRef.current,
                                    nextEl: navigationNextRef.current,
                                }}>
