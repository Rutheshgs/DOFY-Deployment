...
function Questionnaire() {
        ...
        if (getLocalStorage()?.PersonId) {
            ...
            SellOrderServices.Create(orderData).then(res => {
                if (res.status === 200) {
                    ...
                    setTimeout(() => { window.location.href = `/${getUserLanguage()}${getUserLocationForParam(getUserLanguage())}/sell-device-details/${res.data}`; }, 500);
                }
            }).catch((e: string) => {
                console.log(e);
            });
    ...
    const deviceNameHandler = (type: "back" | "brand" | "model" | "variant") => {
        if (type === "brand") {
            ...
            history.push(`/${getUserLanguage()}${getUserLocationForParam(getUserLanguage())}/sell-your-old-device`);
        }
        if (type === "model") {
            dispatch(pageChange("selectbrand"));
            dispatch(InputParamChange({ payload: BrandId, type: ActionType.BRAND_ID }));
            dispatch(DeviceNameChange({ payload: "", type: ActionType.BRAND_ID }));
            history.push(`/${getUserLanguage()}${getUserLocationForParam(getUserLanguage())}/sell-your-old-${deviceName.toLowerCase().replaceAll('_', '-')}`);
        }
        if (type === "variant") {
            dispatch(pageChange("selectmodel"));
            dispatch(InputParamChange({ payload: ModelId, type: ActionType.MODEL_ID }));
            dispatch(DeviceNameChange({ payload: "", type: ActionType.MODEL_ID }));
            history.push(`/${getUserLanguage()}${getUserLocationForParam(getUserLanguage())}/sell-your-old-${deviceName.toLowerCase().replaceAll('_', '-')}/${brandName.toLowerCase().replaceAll('_', '-')}`);
        }
        if (type === "back") {
                            ...
                            <>
                                <IonRow >
                                    <IonCol className='ion-padding-top custom-center'>
                                        <StepProgressBar language={getUserLanguage()} />
                                    </IonCol>
                                </IonRow>
                                <IonRow >
                                    <IonCol className='ion-padding-top custom-center'>
                                                    ...
                                                    <IonGrid style={{ textAlign: 'left' }}>
                                                        ...
                                                        <IonRow>
                                                            {parentQuestions?.map((item: IQuestionTypeModel, index: any) => {
                                                                return (
                                                                    <IonCol onKeyDown={handleKeyDown} className="questions" sizeXl={item && item.AnswerType === questionTypes.POPUP ? "6" : "12"} sizeLg="12" sizeMd="12" sizeXs="12" key={`${sIndex}_${index}`}>
