...
export const CustomAutoComplete = React.forwardRef(({ defaultShow, label, dropDownClassName, customValue, dropDownId, placeholder, data, getId, ...rest }: inputProps, ref) => {
    ...
    return (
        <>
            <ion-item class="bg-color-white cursor-pointer" onClick={() => { setShowSuggestions(true); setFilteredSuggestions(data) }}>
                <ion-label position="fixed">{label}</ion-label>
                <ion-input id="autosearch-input" type="number" onKeyDown={onKeyDown} className="auto-search-input" value={input} onIonChange={(e) => { onChangeValue(e.detail.value); restrictInput(e, 6) }} ref={ref as any} {...rest as any}></ion-input>
            </ion-item>
