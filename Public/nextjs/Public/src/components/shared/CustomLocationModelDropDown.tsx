...
export const CustomLocationModelDropDown = React.forwardRef(({ defaultShow, label, selectedCityHandler, stateData, dropDownClassName, customValue, searchKeyword, selectedCity, setSelectedCity, suggestionsClassName, setSearchKeyWordErr, setSearchKeyWord, setNotValidLocation, dropDownId, placeholder, data, getId, bgColor, ...rest }: inputProps, ref) => {
    ...
    return (
        <>
            {selectedCity ?
                <ion-chip onClick={() => { setSelectedCity(""); selectedCityHandler("") }} color="primary" class="selected-city">
                    <ion-icon icon={locationOutline} color="primary" size="small" />
                    <ion-label>{stateData.find(it => it.Identifier === selectedCity)?.Name}</ion-label>
                    <ion-icon icon={close} />
                </ion-chip>
                :
                <ion-chip color="primary" class="selected-city">
                    <ion-icon icon={locationOutline} color="primary" size="small" />
                    <ion-label>All location</ion-label>
                </ion-chip>
            }
            <ion-item class={`${bgColor === "No" ? "custom-bg" : "bg-color-white"} cursor-pointer ${dropDownClassName}`} onClick={() => { setShowSuggestions(true); setFilteredSuggestions(data) }}>
                {/* {selectedCity ?
                    null
                    :
                    <ion-label position="fixed">{label}</ion-label>
                } */}
                <ion-input id="autosearch-input" type="number" placeholder={label} inputMode="numeric" onKeyDown={onKeyDown} className="auto-search-input" value={input} onIonChange={(e) => { onChangeValue(e.detail.value); restrictInput(e, 6) }} ref={ref as any} {...rest as any}></ion-input>
            </ion-item>
